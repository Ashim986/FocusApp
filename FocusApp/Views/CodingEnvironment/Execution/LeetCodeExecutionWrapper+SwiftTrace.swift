...
extension LeetCodeExecutionWrapper {
    ...
    private static let swiftRunnerTraceBaseLines: [String] = [
        ...
        "                if let charValue = value as? Character { return String(charValue) }",
        "                if let substringValue = value as? Substring { return String(substringValue) }"
    ]

    private static let swiftRunnerTraceTailLines: [String] = [
        "                let typeName = String(describing: type(of: value)).lowercased()",
        "                if typeName.contains(\"stack\") || typeName.contains(\"queue\") {",
        "                    let isStack = typeName.contains(\"stack\")",
        "                    var elements: [Any] = []",
        "                    var foundCollection = false",
        "                    if mirror.displayStyle == .collection {",
        "                        elements = mirror.children.map { $0.value }",
        ...
        "                                elements = childMirror.children.map { $0.value }",
        "                                foundCollection = true",
        "                                break",
        "                            }",
        "                        }",
        ...
        "                    if foundCollection {",
        "                        if structured {",
        "                            let isSimple = elements.allSatisfy { isSimpleValue($0) }",
        "                            let limit = isSimple ? structuredSimpleArrayLimit : structuredComplexArrayLimit",
        "                            if elements.count > limit { didTruncate = true }",
        "                            elements = Array(elements.prefix(limit))",
        "                        }",
        "                        let mapped = elements.map { traceValue($0, structured: structured) }",
        "                        return [\"__type\": isStack ? \"stack\" : \"queue\", \"value\": mapped]",
        "                    }",
        "                }",
        "                if mirror.displayStyle == .tuple {",
        "                    let tupleChildren = Array(mirror.children)",
        "                    let hasLabels = tupleChildren.allSatisfy {",
        "                        if let label = $0.label { return !label.isEmpty }",
        "                        return false",
        "                    }",
        "                    if hasLabels {",
        "                        var mapped: [String: Any] = [:]",
        "                        if structured {",
        "                            let isSimple = tupleChildren.allSatisfy { isSimpleValue($0.value) }",
        "                            let limit = isSimple ? structuredSimpleDictLimit : structuredComplexDictLimit",
        "                            if tupleChildren.count > limit { didTruncate = true }",
        "                            for child in tupleChildren.prefix(limit) {",
        "                                if let label = child.label {",
        "                                    mapped[label] = traceValue(child.value, structured: structured)",
        "                                }",
        "                            }",
        "                            return mapped",
        "                        }",
        "                        for child in tupleChildren {",
        "                            if let label = child.label {",
        "                                mapped[label] = traceValue(child.value, structured: structured)",
        "                            }",
        "                        }",
        ...
        "                        let limit = isSimple ? structuredSimpleArrayLimit : structuredComplexArrayLimit",
        "                        if tupleChildren.count > limit { didTruncate = true }",
        "                        return tupleChildren.prefix(limit).map {",
        "                            traceValue($0.value, structured: structured)",
        "                        }",
        "                    }",
        "                    return tupleChildren.map { traceValue($0.value, structured: structured) }",
        "                }",
        "                if mirror.displayStyle == .set {",
        "                    var setValues = mirror.children.map { $0.value }",
        "                    if structured {",
        "                        let isSimple = setValues.allSatisfy { isSimpleValue($0) }",
        "                        let limit = isSimple ? structuredSimpleArrayLimit : structuredComplexArrayLimit",
        "                        if setValues.count > limit { didTruncate = true }",
        "                        setValues = Array(setValues.prefix(limit))",
        "                    }",
        "                    let mapped = setValues.map { traceValue($0, structured: structured) }",
        "                    return [\"__type\": \"set\", \"value\": mapped]",
        "                }",
        ...
        "                        guard pairChildren.count == 2 else { continue }",
        "                        let key = jsonString(from: pairChildren[0].value)",
        "                        let value = pairChildren[1].value",
        "                        pairs.append((key, value))",
        "                    }",
        "                    pairs.sort { $0.0 < $1.0 }",
        "                    if structured {",
        "                        let isSimple = pairs.allSatisfy { isSimpleValue($0.1) }",
        "                        let limit = isSimple ? structuredSimpleDictLimit : structuredComplexDictLimit",
        "                        if pairs.count > limit { didTruncate = true }",
        ...
        "                    if structured {",
        "                        let isSimple = values.allSatisfy { isSimpleValue($0) }",
        "                        let limit = isSimple ? structuredSimpleArrayLimit : structuredComplexArrayLimit",
        "                        if values.count > limit { didTruncate = true }",
        "                        return values.prefix(limit).map { traceValue($0, structured: structured) }",
        "                    }",
        "                    return values.map { traceValue($0, structured: structured) }",
        "                }",
    ...
    private static let swiftRunnerTraceTemplateEnd = """
            ...
            private static func isSimpleValue(_ value: Any) -> Bool {
                ...
                if value is Character { return true }
                if value is Substring { return true }
                return false
            }
        }
