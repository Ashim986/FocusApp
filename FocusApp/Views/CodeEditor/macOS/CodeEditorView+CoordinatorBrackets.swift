...
extension CodeEditorView.Coordinator {
    ...
    private func applyBracketHighlights(_ ranges: [NSRange], in textView: NSTextView) {
        ...
        var applied: [NSRange] = []
        for range in ranges {
            guard let safeRange = safeTextStorageRange(range, in: textStorage) else { continue }
            textStorage.addAttribute(.backgroundColor, value: highlightColor, range: safeRange)
            textStorage.addAttribute(
                .underlineStyle,
                value: NSUnderlineStyle.single.rawValue,
                range: safeRange
            )
            textStorage.addAttribute(.underlineColor, value: underlineColor, range: safeRange)
            applied.append(safeRange)
        }
        bracketHighlightRanges = applied
    }
    ...
    private func safeTextStorageRange(_ range: NSRange, in textStorage: NSTextStorage) -> NSRange? {
        let length = textStorage.length
        guard length > 0 else { return nil }
        guard range.location >= 0, range.location < length else { return nil }
        let end = min(NSMaxRange(range), length)
        let clampedLength = end - range.location
